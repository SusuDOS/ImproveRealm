...
添加 `^~` 显示 `abcd ccess success`
没有添加则显示 `abc ccess success`
...
server {
	...
	location ^~/abcd {
		default_type text/plain;
		return 200 "abcd ccess success";
	}
    location ~*^/abc{
		default_type text/plain;
		return 200 "abc ccess success";
	}
}
```
...
http{
    ...
    gzip_types *;
    gzip_comp_level 6;
    # 长度大于280364则压缩，小于和等于不压缩.
    gzip_min_length 280364;
}
```
...
(3)再次访问`http://192.168.200.133/jquery.js`,发现同名带gzip的文件则返回带gzip的文件，没有则返回未尽压缩的文件，nginx.conf不用配置压缩选项。
...
![缓冲.png](assets/缓冲.png)
![请求头添加.png](assets/请求头添加.png)
请求头添加.png
